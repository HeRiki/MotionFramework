...
namespace MotionFramework.Config
{
	...
	public abstract class AssetConfig
	{
		...
		private void Handle_Completed(AssetOperationHandle obj)
		{
			...
			{
				MotionLog.Log(ELogType.Error, $"Failed to parse config {Location}. Error : {ex.ToString()}");
			}
		...
		public ConfigTab GetTab(int key)
		{
			...
			{
				MotionLog.Log(ELogType.Warning, $"Faild to get tab. File is {Location}, key is {key}");
				return null;
			}
		}
